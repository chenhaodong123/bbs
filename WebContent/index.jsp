		...
		<TABLE cellSpacing="0" cellPadding="0" width="100%">
			...
			<TR class="tr3">
				...
				<TH align="left"><IMG src="image/board.gif"> <A
					href="servletListPage?sid=<%=listChild.get(j).getSid()%>"> <%=listChild.get(j).getSname()%></A>
				</TH>
				<TD align="center"><%=listChild.get(j).getStopiccount()%></TD>
				<TH>
					<%
				...
				if (temp1 != null) {
					...
					if(temp2 != null){
				%> <SPAN><A
						href="servletDetailPage?tid=<%=temp2.getTid()%>
					&sid=<%=sId%>&action=showDetail"><%=temp2.getTitle()%></A></SPAN>
					<BR /> <SPAN><%=temp2.getAuthor()%></SPAN> <SPAN class="gray">
