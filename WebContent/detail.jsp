...
<BODY>
	<table height="50" width="100%" border="0" cellspacing="0"
		cellpadding="0"
		style="border-collapse: collapse; border: #DBDBDB; margin-top: 10px; background-color: #dff0f8;">
		<tr>
			<td align="center"><font style="font-size: 28px; color: #3982a4">校园论坛</font></td>
		</tr>
	</table>
	<DIV></DIV>
	...
	<DIV>
		...
		<!--      导航        -->
		<DIV>
			&gt;&gt;<B><a href="index.jsp">论坛首页</a></B><%@ include
				file="navigation.jsp"%>
		</DIV>
		<br />
		...
		<!--      回复、新帖        -->
		<DIV>
			<A href="post.jsp?tid=<%=tid%>&sid=<%=sid%>&action=reply"><IMG
				src="image/reply.gif" border="0" id=td_post></A> <A
				href="post.jsp?tid=<%=tid%>&sid=<%=sid%>&action=post"><IMG
				src="image/post.gif" border="0" id=td_post></A>
		</DIV>
		...
		<DIV class="pages">
			...
			<a
				href="servletDetailPage?tid=<%=tid%>&sid=<%=sid%>&action=showPage&currPage=<%=--PreviousPage%>">上一页</a>&nbsp;|&nbsp;
			<a
				href="servletDetailPage?tid=<%=tid%>&sid=<%=sid%>&action=showPage&currPage=<%=++nextPage%>">下一页</a>&nbsp;|&nbsp;
			<%
		if(PageCount > 0){
	%>
			<a
				href="servletDetailPage?tid=<%=tid%>&sid=<%=sid%>&action=showPage&currPage=1"><<</a>&nbsp;
	...
	%>
			<a
				href="servletDetailPage?tid=<%=tid%>&sid=<%=sid%>&action=showPage&currPage=<%=i%>"><%=i%></a>&nbsp;
			<%
		} 
	%>
			<%-- 实现分页 打印代码一 结束 --%>
			<input style="width: 30px; height: 14px" type="text"
				onkeydown="javascript: if(event.keyCode==13){ location='servletDetailPage?tid=<%=tid%>&sid=<%=sid%>&action=showPage&currPage='+this.value+'';return false;}"\>
			<a
				href="servletDetailPage?tid=<%=tid%>&sid=<%=sid%>&action=showPage&currPage=<%=PageCount%>">>></a>&nbsp;
	...
	%>
		...
		<DIV class="t">
			<TABLE style="BORDER-TOP-WIDTH: 0px; TABLE-LAYOUT: fixed"
				...
				<TR class="tr1">
					<TH style="WIDTH: 20%"><b>#1</b><br /> <B><%=topicObj.getName()%></B><BR />
						组别：<%=user_dao.getUserTypeNameById(topicObj.getType()) %><br />
						性别：<%=user_dao.getSexName(topicObj.getSex()) %><br /> <img
						src="image/head/<%=topicObj.getFace()%>" /><BR /> 注册:<%=topicObj.getRegtime()%><BR />
					</TH>
					<TH>
			...
			%>
							<A
								href="servletDetailPage?tid=<%=tid%>&sid=<%=sid%>&action=delTopic"
			...
			%>
							<A
								href="servletDetailPage?tid=<%=tid%>&sid=<%=sid%>
	...
	%>
		<DIV class="t">
			<TABLE style="BORDER-TOP-WIDTH: 0px; TABLE-LAYOUT: fixed"
				cellSpacing="0" cellPadding="0" width="100%">
				<TR class="tr1">
					<TH style="WIDTH: 20%"><b>#<%=i+2%></b><br /> <B><%=replyList.get(i).getName()%></B><BR />
						组别：<%=user_dao.getUserTypeNameById(replyList.get(i).getType()) %><BR />
						性别：<%=user_dao.getSexName(replyList.get(i).getSex()) %><br /> <img
						src="image/head/<%=replyList.get(i).getFace()%>" /><BR /> 注册:<%=replyList.get(i).getRegtime()%><BR />
					</TH>
					<TH>
			...
			%>
							<A
								href="servletDetailPage?tid=<%=tid%>&sid=<%=sid%>&rid=
