...
<BODY>
	...
	%>
		<DIV>
			<FORM name="updateForm" onSubmit="return check(1);"
				action="servletDetailPage?tid=<%=tid%>&sid=<%=sid%>&action=updateTopic"
				method="post">
				<INPUT type="hidden" name="boardId" value="8" /> <INPUT
					type="hidden" name="replyId" value="61" />
				<DIV class="t">
					<TABLE cellSpacing="0" cellPadding="0" align="center">
						...
						<TR class="tr3">
							...
							<TH><INPUT name="title" value="<%=edittopic.getTtopic()%>"
							...
							<TH colSpan="2">
								<DIV>
									<span><textarea name="content"
											style="WIDTH: 500px; word-break: break-all;" rows="20"
											cols="90" tabIndex="2"><%=edittopic.getTcontents() %></textarea></span>
	...
	%>
		<DIV>
			<FORM name="updateForm" onSubmit="return check(0);"
				action="servletDetailPage?tid=<%=tid%>&sid=<%=sid%>&rid=<%=editreply.getRid()%>&action=updateReply"
				method="post">
				<INPUT type="hidden" name="boardId" value="8" /> <INPUT
					type="hidden" name="replyId" value="61" />
				<DIV class="t">
					<TABLE cellSpacing="0" cellPadding="0" align="center">
						...
						<TR class="tr3">
							<TH width="20%"><B>标题</B></TH>
							<TH><INPUT name="title" value="<%=editreply.getRtopic()%>"
								class="input" style="PADDING-LEFT: 2px; FONT: 14px Tahoma"
								tabIndex="1" size="60"></TH>
						</TR>
						...
						<TR class="tr3">
							...
							<TH colSpan="2">
								<DIV>
									<span><textarea name="content"
											style="WIDTH: 500px; word-break: break-all;" rows="20"
											cols="90" tabIndex="2"><%=editreply.getRcontents() %></textarea></span>
								</DIV> (不能大于:<FONT color="blue">1000</FONT>字)
							</TH>
						</TR>
					</TABLE>
